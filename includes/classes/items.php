...
class items
{
  public static function get_choices_by_entity($entity_id,$parent_entity_id)
  {            
    $listing_sql_query = '';
  ...
  public static function add_access_query($current_entity_id,$listing_sql_query)
  {
    ...
    {                    
      ...
      if(count($users_fields)>0 or count($grouped_users_fields)>0)
      {
        ...
        {                    
          $sql_query_array[] = "(select count(*) as total from app_choices_values cv where cv.entities_id='" . $current_entity_id . "' and cv.items_id=e.id and cv.fields_id='" . $id . "' and cv.value='" . $app_user['id'] . "')>0";          
        }
        
        foreach($grouped_users_fields as $id)
        {        
          $sql_query_array[] = "(select count(*) as total from app_choices_values cv where cv.entities_id='" . $current_entity_id . "' and cv.items_id=e.id and cv.fields_id='" . $id . "' and cv.value in (select id from app_fields_choices fc where fc.fields_id='" . $id . "' and find_in_set(" . $app_user['id']  . ",fc.users)))>0";                                
        }
