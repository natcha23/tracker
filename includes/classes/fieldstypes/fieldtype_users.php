...
class fieldtype_users
{
  ...
  function reports_query($options)
  {
    ...
    {
      $sql_query[] = "(select count(*) from app_choices_values as cv where cv.entities_id='" . db_input($options['entities_id']) . "' and cv.items_id=e.id and cv.fields_id='" . db_input($options['filters']['fields_id'])  . "' and cv.value in (" . $filters['filters_values'] . ")) " . ($filters['filters_condition']=='include' ? '>0': '=0');
    }
