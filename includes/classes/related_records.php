  ...
  <div class="related_itmes_listing">  
    <div class="table-scrollable">
      <div class="table-scrollable table-wrapper">
        <table class="table table-striped table-bordered table-hover" data-count-fixed-columns="0">
          <thead>
            <tr>
              ';
...
//render listing body
$listing_sql = "select e.* " . $listing_sql_query_select . " from app_entity_" . $this->cfg->get('entity_id') . " e "  . $listing_sql_query_join . " where e.id>0 " . $listing_sql_query;
...
while($item = db_fetch_array($items_query))
{
  ...
  if(users::has_access('update',$this->current_entities_access_schema))
  {
    $html .= '
          <td>
            <a onClick="return confirm(i18n[\'TEXT_ARE_YOU_SURE\'])" href="' . url_for('items/related_item','action=remove_related_item&path=' . $current_path . '&id=' . $related_id). '" title="' . TEXT_BUTTON_DELETE_RELATION . '" class="btn btn-default btn-xs btn-xs-fa-centered"><i class="fa fa-chain-broken"></i></a>
          </td>';
  }
...
if($has_numeric_fields)
{
  ...
  foreach($listing_fields as $field)
  {
    if(isset($fields_totals_array[$field['id']]))
    {
      $html .= '<td>' . $fields_totals_array[$field['id']] . '</td>';
    }
    else
    {
      $html .= '<td></td>';
  ...
  {        
    ...
    if($this->cfg->get('entity_id')==$this->field['entities_id'])
    {
    	$related_items_query = db_query("select id, if(related_items_id=" . db_input($this->items_id) . ",items_id,related_items_id) as related_items_id from app_related_items where (entities_id = '"  . db_input($this->entities_id) . "' and items_id = '" . db_input($this->items_id) . "' and related_entities_id='" . db_input($this->cfg->get('entity_id')) . "')
	                                    or (related_entities_id='" . db_input($this->entities_id) . "' and related_items_id='" . db_input($this->items_id) . "' and entities_id = '"  . db_input($this->cfg->get('entity_id')) . "')");
    }
    else 
    {
	    $related_items_query = db_query("select id, if(related_entities_id=" . db_input($this->cfg->get('entity_id')) . ",related_items_id,items_id) as related_items_id from app_related_items where (entities_id = '"  . db_input($this->entities_id) . "' and items_id = '" . db_input($this->items_id) . "' and related_entities_id='" . db_input($this->cfg->get('entity_id')) . "') 
	                                    or (related_entities_id='" . db_input($this->entities_id) . "' and related_items_id='" . db_input($this->items_id) . "' and entities_id = '"  . db_input($this->cfg->get('entity_id')) . "')");
    }
    while($related_items = db_fetch_array($related_items_query))
    {
      $related_items_array[$related_items['id']] = $related_items['related_items_id'];
    }
  ...
  {        
    ...
    /*
    $related_items_query = db_query("select count(*) as total from app_related_items where (entities_id = '"  . db_input($this->entities_id) . "' and items_id = '" . db_input($this->items_id) . "' and related_entities_id='" . db_input($this->cfg->get('entity_id')) . "') 
                                    or (related_entities_id='" . db_input($this->entities_id) . "' and related_items_id='" . db_input($this->items_id) . "' and entities_id = '"  . db_input($this->cfg->get('entity_id')) . "')");
    $related_items = db_fetch_array($related_items_query);
    */
    ...
    {  
      ...
      $listing_sql = "select count(*) as total from app_entity_" . $this->cfg->get('entity_id') . " e where e.id>0 " . $listing_sql_query;
  ...
  {
    $related_items_query = db_query("select * from app_related_items where (entities_id = '"  . db_input($entities_id) . "' and items_id = '" . db_input($items_id) . "') or (related_entities_id = '"  . db_input($entities_id) . "' and related_items_id = '" . db_input($items_id) . "')");
    while($related_items = db_fetch_array($related_items_query))
    {
      db_delete_row('app_related_items',$related_items['id']); 
    }  
  }
