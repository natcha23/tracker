...
class users
{
  ...
  static public function has_access_to_assigned_item($entities_id, $items_id)
  {
    ...
    if(count($users_fields)>0 or count($grouped_users_fields)>0)
    {
      ...
      foreach($users_fields as $id)
      {        
        $sql_query_array[] = "(select count(*) as total from app_choices_values cv where cv.entities_id='" . $entities_id . "' and cv.items_id=e.id and cv.fields_id='" . $id . "' and cv.value='" . $app_user['id'] . "')>0";
      }
      
      foreach($grouped_users_fields as $id)
      {                
        $sql_query_array[] = "(select count(*) as total from app_choices_values cv where cv.entities_id='" . $entities_id . "' and cv.items_id=e.id and cv.fields_id='" . $id . "' and cv.value in (select id from app_fields_choices fc where fc.fields_id='" . $id . "' and find_in_set(" . $app_user['id']  . ",fc.users)))>0";
      }
