...
class hot_reports
{
    ...
    while($reports = db_fetch_array($reports_query))
    {
      $html .= '
        <li class="dropdown hot-reports" id="hot_reports_' . $reports['id'] . '" data-id="' . $reports['id'] . '">
          ' . $this->render_dropdown($reports['id']) . '
        </li>
  ...
  function get_items($report_info)
  {
    $listing_sql_query = '';
    ...
    $count_items_sql = "select count(*) as total from app_entity_" . $report_info['entities_id'] . " e "  . $listing_sql_query_join . " where e.id>0 " . $listing_sql_query . " ";
    $count_items_query = db_query($count_items_sql);
    $count_items = db_fetch_array($count_items_query);
    
    $count = 0;
    $listing_sql = "select e.* from app_entity_" . $report_info['entities_id'] . " e "  . $listing_sql_query_join . " where e.id>0 " . $listing_sql_query . " ";
    ...
    return array('items_count'=> $count_items['total'],'items_array'=>$items_array);
  }
